...
export class Client implements IClient {
  ...
  async make(method: HttpMethod, path: string): Promise<{ req: superagent.SuperAgentRequest; }> {
  ...
  async do(req: superagent.SuperAgentRequest): Promise<APIResponseSuccess> {
    const res = await req;
...
export abstract class ResourceClient {
  protected applyModifiers(req: superagent.Request, modifiers?: ResourceClientRequestModifiers) {
    if (!modifiers) {
      return;
    }
  ...
  protected applyAuthentication(req: superagent.Request, token: string) {
    req.set('Authorization', `Bearer ${token}`);
  }
}

export function transformAPIResponse(r: superagent.Response): APIResponse {
  if (r.status === 204) {
...
function formatAPIResponse(req: superagent.SuperAgentRequest, r: APIResponse): string {
  return formatResponseError(req, r.meta.status, isAPIResponseSuccess(r) ? r.data : r.error);
}

export function formatResponseError(req: superagent.SuperAgentRequest, status?: number, body?: object | string): string {
  return chalk.red(
